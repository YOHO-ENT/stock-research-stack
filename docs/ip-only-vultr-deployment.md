...
or deploy moomoo account services publicly.
...
| Moomoo account services | Local-only; no Vultr route |
...
- upload committed sibling repo source archives from the local machine into
  `/opt/market-data-lab`, `/opt/Firn`, and `/opt/TradingAgents`, so the VPS
  does not need GitHub credentials for private repos;
- remove any old `/opt/py-moomoo-api` source tree and
  `research-stack_moomoo_account_web_data` Docker volume;
...
curl -I http://127.0.0.1:8080/moomoo/
```
...
- Market Data Lab, Firn, and TradingAgents are routed behind the same Basic Auth
  gate.
- `/moomoo/` is not routed; the internal Caddy check should return `404`.
- moomoo Account Web and OpenD `11111` remain local-only and are not published
  by Caddy or Docker.
