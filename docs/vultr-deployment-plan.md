...
deploy moomoo account services to the VPS.
...
- Compose starts Research Hub, Caddy, Market Data Lab API/UI, Firn API/UI, and
  TradingAgents API/UI.
- DailyBrief remains a systemd timer/publisher and is not moved into Compose.
- py-moomoo-api and moomoo OpenD stay local-only and are not deployed to Vultr.
...
- `py-moomoo-api` depends on local moomoo OpenD for live account reads. It is
  intentionally not deployed to Vultr, and `/moomoo/` is not routed publicly.
- `market-data-lab` sets `MOOMOO_ACCOUNT_WEB_URL` empty in production so
  moomoo preview/sync fails fast on the VPS.
...
- Caddy internal checks pass for Hub, DailyBrief, Market Data Lab, Firn, and
  TradingAgents.
- moomoo Account Web and OpenD stay local-only unless a separate security
